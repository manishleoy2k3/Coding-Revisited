...
public class SortArrayIncreasing_Frequency {
	...
	public static int[] frequencySort(int[] nums) {
		...
		/*
		 * Map<Integer,Integer> m=new HashMap<>(); for(int i=0;i<nums.length;i++){
		 * if(m.containsKey(nums[i])) m.put(nums[i],m.get(nums[i])+1); else
		 * m.put(nums[i],1); }
		 * 
		 ...
		 * System.out.print(nums[a]); } return nums;
		 */
