...
public class Google_TimeToTypeString {
	...
	public static void main(String[] args) {
        ...
        int diff = hashmap.get(text.charAt(0));
        for(int i=1; i<text.length(); i++) {
            int n = hashmap.get(text.charAt(i));
            int m = hashmap.get(text.charAt(i-1));
            //System.out.println("n= " +n+ "m= "+m);
            diff = diff + Math.abs(n-m);
        }
		System.out.println(diff);
		
		/*int diff =0, initial=0, current=0;
		for(int i=0; i<= text.length()-1; i++){
			System.out.println("initial is: " + initial);
			System.out.println("current is: " + current);
			System.out.println("diff is: "+diff);
			
			current = Math.abs(keyboard.indexOf(text.charAt(i)) - initial );
			diff = diff + Math.abs(current - initial);
			initial = current;
